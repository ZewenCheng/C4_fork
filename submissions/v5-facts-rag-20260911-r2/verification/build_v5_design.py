...
def footer(c,doc):
    ...
    c.drawString(48,28,'版本 v5-facts-rag-20260911-r2 | 方案设计与验证边界')
...
def main():
    ...
    fix=json.loads((ROOT/'tmp/v5-entryfix.json').read_text(encoding='utf-8'));assert fix['退出码']==0
    ...
    story += [PageBreak(),p('全量实测与部署交付','title'),p('5 本轮330图实测','h2'),
        ...
        p('r2仅修复启动脚本：在SDK环境初始化后启用严格变量检查，已验证未预设LD_LIBRARY_PATH时可运行。Python推理代码、模型及本轮330图结果保持原样，修复后未重复模型推理；运行来源与包装源码分别绑定。'),
