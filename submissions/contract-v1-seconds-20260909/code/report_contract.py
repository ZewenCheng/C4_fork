...
def locked_diagnosis(row, packet):
    ...
    return {'questionCategory': category, 'bridgeName': path.parent.name, 'filename': path.name,
            'defectType': label.strip(), 'source': 'inspect_semantics/adamw/top1', 'is_intact': intact(label)}
...
def validate_prediction_source(diagnosis, catalog):
    source = catalog.get('inspect_semantics/adamw/0')
...
def parse_grade(raw, diagnosis, catalog):
    if diagnosis['is_intact']:
        return {'rating': '', 'reason': '按完好评级留空合同执行', 'evidence_ids': ['inspect_semantics/adamw/0'], 'kind': 'intact_policy'}
...
def assemble_final(diagnosis, grade, catalog):
    ...
    location = '、'.join(locations) + '（候选定位）' if locations else ''
    if diagnosis['is_intact']:
...
def validate_final(final, diagnosis):
    ...
    if not final['bridgeName'] or not final['defectDescription']:
        raise ContractError('目录名称或确定性描述缺失')
    if diagnosis['is_intact']:
...
def make_context(row, packet, producer):
    return {'protocol': VERSION, 'sample_id': safe_sample_id(row['sample_id']),
...
def complete_record(row, packet, producer, grade, attempts):
    ...
    return {'status': 'complete', 'context': make_context(row, packet, producer), 'locked_diagnosis': diagnosis,
            ...
            'field_owners': {'identity': 'input_directory', 'defectType': diagnosis['source'],
                             'defectLocation': 'mapped_region_candidates', 'defectDescription': 'deterministic_evidence_template',
