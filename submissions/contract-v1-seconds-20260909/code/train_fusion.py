...
def main():
    ...
    for name in ['adamw','sgd_momentum']:
        ...
        dump(status,{'status':'loading_fusion_features','optimizer':name,'time':time.time()})
        features=[]
        for p in folders:
            vectors=None
            for index,row in enumerate(rows):
                with np.load(p/(row['sample_id']+'.npz')) as d:
                    ...
                    vector=F.normalize(F.normalize(f,dim=-1).mean(0),dim=-1)
                    if vectors is None:vectors=torch.empty((len(rows),len(vector)),dtype=torch.float32)
                    vectors[index]=vector
                if index%200==0:dump(status,{'status':'loading_fusion_features','optimizer':name,'source':p.name,'completed':index+1,'total':len(rows),'time':time.time()})
