...
class QwenToolRouter:
    ...
    def run(self,messages):
        ...
        history=list(messages);events=[];count=0;start=time.monotonic();evidence={}
        required={'inspect_semantics','inspect_regions','inspect_masks','inspect_grounding'}
        while True:
            final_stage=required.issubset(evidence) or count>=8
            if final_stage:
                history=[{'role':'system','content':'你处于报告生成阶段，工具已关闭。仅依据提供的专家证据生成七字段JSON。所有值为字符串，缺失信息为空字符串；不把分数或网格面积换成等级、物理尺寸，不编造桥名或位置。字段为questionCategory, bridgeName, defectLocation, filename, defectType, defectDescription, ratingScale(1-5)。最终只输出JSON，不输出工具调用。'},messages[-1],{'role':'user','content':json.dumps({'expert_evidence':{k:v for k,v in evidence.items() if k in required},'missing_experts':sorted(required-set(evidence))},ensure_ascii=False)}]
            prompt=self.tokenizer.apply_chat_template(history,tools=None if final_stage else self.tools,tokenize=False,add_generation_prompt=True,enable_thinking=False)
            ...
            if final_stage and calls:return {'status':'tool_call_in_final_stage','events':events,'final':None,'raw_text':text}
            if not calls:
            ...
            for call in calls:
                ...
                if result['status']=='ok':evidence[function['name']]=result
