...
def prefetch_inputs(rows):
    def load(row):
        sid=row['sample_id'];features={};regions={}
        for family in ['hplus_distilled_features','hplus_original_features','wemm_features','legacy_convnext_features']:
            for opt in (OPTS if family!='legacy_convnext_features' else ['shared']):
                folder=ONLINE/family if opt=='shared' else ONLINE/family/opt
                with np.load(folder/(sid+'.npz'),allow_pickle=False) as z:
                    features[(family,opt)]={k:z[k] for k in ['features','scores'] if k in z}
        for family in ['rtdetr_holdout','sam_holdout','grounding_holdout']:
            for opt in OPTS:
                folder=ONLINE/family/opt;packet=json.loads((folder/(sid+'.json')).read_text())
                item={'packet':packet,'complete':json.loads((folder/'complete.json').read_text())}
                if family=='sam_holdout':
                    raw=(folder/packet['mask_file']).read_bytes()
                    item['mask_sha256']=hashlib.sha256(raw).hexdigest()
                    with np.load(io.BytesIO(raw),allow_pickle=False) as z:item['masks']=z['masks']
                regions[(family,opt)]=item
        return {'features':features,'regions':regions,'anomaly':json.loads((ONLINE/'anomaly_holdout'/(sid+'.json')).read_text())['result']}
    with ThreadPoolExecutor(max_workers=4) as pool:
        pending={i:pool.submit(load,row) for i,row in enumerate(rows[:8])}
        for i in range(len(rows)):
            result=pending.pop(i).result()
            if i+8<len(rows):pending[i+8]=pool.submit(load,rows[i+8])
            yield rows[i],result
...
def main():
    ...
    cv_classifier=CVMultiviewClassifier(cv_root/'multiview_model.npz',cv_root/'manifest.json',labels,legacy_source)
    packets=[];input_facts=[]
    bank=json.loads((ROOT/'checkpoints/anomaly_banks/configuration.json').read_text())
    for i,(row,loaded) in enumerate(prefetch_inputs(rows)):
        ...
        for opt in OPTS:
            ...
            for family in ['hplus_distilled_features','wemm_features','legacy_convnext_features']:
                ...
                f=torch.from_numpy(loaded['features'][(family,'shared' if family=='legacy_convnext_features' else opt)]['features'].astype(np.float32))
            ...
            for family in ['hplus_distilled_features','hplus_original_features']:
                sc=loaded['features'][(family,opt)]['scores']
        ...
        cv_output=cv_classifier.predict(loaded['features'][('legacy_convnext_features','shared')]['features'])
        add('inspect_cv_multiview',{'sample_id':sid,'outputs':{'multiview':cv_output}})
        selected_label=cv_output['candidates'][0]['label']
        assets['cv_multiview']=cv_output['checkpoint_sha256']
        for tool,family,prefix in [('inspect_regions','rtdetr_holdout','rtdetr_'),('inspect_masks','sam_holdout','sam3_'),('inspect_grounding','grounding_holdout','grounding_dino_')]:
            outputs={}
            for opt in OPTS:
                folder=ONLINE/family/opt;p=folder/(sid+'.json');item=loaded['regions'][(family,opt)];packet=item['packet'];_,digest=checked(ROOT/'checkpoints'/(prefix+opt));assert packet['sample_id']==sid and packet['checkpoint_sha256']==digest
                assert item['complete']['checkpoint_sha256']==digest;assets[prefix+opt]=digest
                if tool=='inspect_masks':
                    assert item['mask_sha256']==packet['mask_sha256']
                    m=item['masks']
                ...
                packet['candidates']=select_region_candidates(packet['candidates'],selected_label,limit=5)
                packet['selection_policy']='report-contract-v1:类型对应与查询覆盖优先，预算5，不改变分类或候选分数'
                outputs[opt]=packet
            add(tool,{'sample_id':sid,'outputs':outputs})
        anomaly=loaded['anomaly'];assert anomaly['sample_id']==sid
        ...
        packets.append({'sample_id':sid,'filename':Path(row['path']).name,'category':category,'predicted_type':label,'prediction_source':'inspect_cv_multiview/multiview/top1','evidence':evidence,'events':events})
