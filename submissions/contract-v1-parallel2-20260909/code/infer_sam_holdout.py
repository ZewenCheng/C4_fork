...
def prepare_text_cache(model,processor,queries):
    """每份冻结权重仅编码一次全局词表；不跨优化器共享嵌入。"""
    if model.training or torch.is_grad_enabled():raise ValueError('文本复用仅允许冻结推理')
    cache={}
    for query in queries:
        inputs=processor(text=query,return_tensors='pt').to('cuda')
        with torch.autocast('cuda',dtype=torch.bfloat16):
            embeds=model.get_text_features(input_ids=inputs['input_ids'],attention_mask=inputs.get('attention_mask'),return_dict=True).pooler_output
        cache[query]=(embeds,inputs.get('attention_mask'))
    return cache

def predict_queries(model,processor,image,queries,text_cache):
    """同一图像只计算一次图像编码，查询的独立解码与原始次序保持。"""
    if model.training or torch.is_grad_enabled() or set(queries)!=set(text_cache):raise ValueError('图像复用条件不符')
    pixels=processor(images=image,return_tensors='pt').to('cuda')['pixel_values']
    with torch.autocast('cuda',dtype=torch.bfloat16):vision=model.get_vision_features(pixel_values=pixels)
    del pixels
    for query in queries:
        text,mask=text_cache[query]
        with torch.autocast('cuda',dtype=torch.bfloat16):
            prediction=model(vision_embeds=vision,text_embeds=text,attention_mask=mask)
        yield query,prediction
...
def main():
    ...
    for name in ['adamw','sgd_momentum']:
        ...
        with torch.inference_mode():
            text_cache=prepare_text_cache(model,processor,queries)
            for i,row in enumerate(rows):
                ...
                candidates=[];packed=[]
                for query,prediction in predict_queries(model,processor,im,queries,text_cache):
        ...
        del text_cache,model,base;gc.collect();torch.cuda.empty_cache()
