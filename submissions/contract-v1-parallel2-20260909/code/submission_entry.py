...
def prepare(code, work, inp, resume=False, check_only=False, execution=None):
    ...
    receipt = {'protocol': 'c4-submission-contract-v1', 'input': str(inp), 'work': str(work),
               ...
               'assets': asset_paths, 'execution': execution or {}}
    if work.exists():
...
def main():
    ...
    parser.add_argument('--report-batch-size', type=int, choices=range(1, 9), help='默认采用发布清单中平台实测选定的批量')
    parser.add_argument('--cpu-workers', type=int, choices=[1, 2, 4, 8], default=4)
    parser.add_argument('--attention-backend', choices=['sdpa', 'flash_attention_2', 'c4_unpadded_sdpa'], help='默认采用发布清单固定的注意力后端')
    args = parser.parse_args()
    if args.resume and args.work is None:
        parser.error('恢复必须明确--work')
    code = Path(__file__).resolve().parent
    published_execution = read(code / 'source_manifest.json').get('execution', {})
    if args.report_batch_size is None:
        args.report_batch_size = published_execution.get('report_batch_size', 4)
    if args.report_batch_size not in range(1, 9):
        parser.error('发布清单中的报告批量无效')
    if args.attention_backend is None:
        args.attention_backend = published_execution.get('attention_backend', 'sdpa')
    if args.attention_backend not in ['sdpa', 'flash_attention_2', 'c4_unpadded_sdpa']:
        parser.error('发布清单中的注意力后端无效')
    work = args.work or WORK_BOUNDARY / ('c4-contract-run-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ') + '-' + uuid.uuid4().hex[:8])
    execution = {'report_batch_size': args.report_batch_size, 'cpu_workers': args.cpu_workers, 'attention_backend': args.attention_backend}
    work, receipt = prepare(code, work, Path(args.input), args.resume, args.check, execution)
    if args.check:
        ...
        try:
            for name in ['start_online_candidate.py', 'online_full_pipeline.py']:
                subprocess.run([sys.executable, '-B', '-u', str(work / name)], cwd=work,
                               env=dict(os.environ, PYTHONPATH=str(work), TABPFN_DISABLE_TELEMETRY='1',
                                        C4_REPORT_BATCH_SIZE=str(args.report_batch_size), C4_CPU_WORKERS=str(args.cpu_workers),
                                        C4_ATTENTION_BACKEND=args.attention_backend), check=True)
