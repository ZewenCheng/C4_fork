...
def main():
    ...
    with run_lock(OUT/'run.lock'):
        ...
        config={'source_input_sha256':digest_file(SOURCE/'input_manifest.json'),'selection_sha256':selection,'samples':8,
                'candidates':[1,2,4,8],'precision':'bfloat16','attention':'c4_unpadded_sdpa','max_new_tokens':512,
        ...
        try:
            ...
            for size in [1,2,4,8]:
                ...
                try:
                    ...
                    records.append({'batch_size':size,'attention_backend':'c4_unpadded_sdpa','skipped':'超出本轮显存预算，不作为可用批量','phase':phase})
