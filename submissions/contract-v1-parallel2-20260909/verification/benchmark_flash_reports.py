"""用原固定8条病害串行输出核验平台现有FlashAttention批量吞吐；逐图数据不导出。"""
import time
...
def main():
    ...
    with run_lock(OUT/'run.lock'):
        ...
        config={'source_input_sha256':digest_file(SOURCE/'input_manifest.json'),'selection_sha256':selection,'samples':8,
                'candidates':[1,2,4,8],'precision':'bfloat16','attention':'flash_attention_2','max_new_tokens':512,
        ...
        worker=threading.Thread(target=monitor,daemon=True);worker.start();grader=FrozenGrader('flash_attention_2',oom_fallback=False)
        records=[baseline];raw_outputs={}
        ...
        def measure(size,label):
            ...
            record={'batch_size':size,'attention_backend':'flash_attention_2','seconds':seconds,'images_per_second':8/seconds,
        ...
        try:
            ...
            for size in [1,2,4,8]:
                phase='warmup_flash_batch_'+str(size);status(batch_size=size)
                try:
                    grader.generate_batch(requests[:size])
                    measure(size,'measure_flash_batch_'+str(size))
                except RuntimeError as exc:
                    if not isinstance(exc,torch.OutOfMemoryError) and 'out of memory' not in str(exc).lower():raise
                    records.append({'batch_size':size,'attention_backend':'flash_attention_2','skipped':'超出本轮显存预算，不作为可用批量','phase':phase})
                gc.collect();torch.cuda.empty_cache()
            valid=[v for v in records if v.get('attention_backend')=='flash_attention_2' and 'seconds' in v and not v['protocol_failures'] and not v['final_differences_vs_serial']]
            if not valid:raise RuntimeError('现有FlashAttention无通过原串行结果对照的配置')
            chosen=min(valid,key=lambda v:v['seconds']);confirmation=measure(chosen['batch_size'],'confirm_selected_batch')
            ...
            summary={'状态':'同输入串行批量对照通过','配置':config,'模型加载秒':grader.model_load_seconds,'选定批量':chosen['batch_size'],
                     '选定注意力后端':'flash_attention_2','对照':records,'确认批次相对串行加速比':baseline['seconds']/confirmation['seconds'],
