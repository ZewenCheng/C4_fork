...
def main():
    ...
    status=ROOT/'candidate_online_20260908/grounding_inference_status.json'
    ...
    for name in ['adamw','sgd_momentum']:
        source=ROOT/'checkpoints'/('grounding_dino_'+name);out=ROOT/'candidate_online_20260908/grounding_holdout'/name;out.mkdir(parents=True,exist_ok=True)
        ...
        with torch.inference_mode():
            for i,row in enumerate(rows):
                ...
                for query in queries:
                    inputs=processor(images=[im],text=query+' .',return_tensors='pt').to('cuda');prediction=model(**inputs)
...
if __name__=='__main__':
    try:main()
    except Exception as e:dump(ROOT/'candidate_online_20260908/grounding_inference_status.json',{'status':'failed','error_type':type(e).__name__,'time':time.time()});raise
