...
def main():
    out=ROOT/'candidate_online_20260908/reports';out.mkdir(exist_ok=True,parents=True)
    status=ROOT/'candidate_online_20260908/qwen_report_status.json'
    rows=[r for r in json.loads((ROOT/'candidate_online_20260908/input_manifest.json').read_text()) if r['split']=='holdout']
    ...
    dump(out/'configuration.json',{'runner_sha256':sha(Path(__file__)),'manifest_sha256':sha(ROOT/'candidate_online_20260908/input_manifest.json'),'model':'/model/Qwen3.6-27B','cpu_workers':4,'batch_initial':2,'fields':FIELDS,'protocol':'必需专家由网关调用，冻结Qwen批量编排；独立版本，不作正式提交'})
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        while position<len(pending):
            ...
            dump(status,{'status':'generating_whole_holdout_reports','version':'online_full_v1','completed':done+processed,'total':len(rows),'images_per_second':processed/(time.monotonic()-begin),'cpu_workers':4,'batch_size':batch_size,'recent_batches':batch_history[-10:],'time':time.time()})
    dump(status,{'status':'whole_holdout_drafts_complete_effectiveness_review_pending','version':'online_full_v1','images':len(rows),'time':time.time()})

if __name__=='__main__':
    try:main()
    except Exception as e:
        dump(ROOT/'candidate_online_20260908/qwen_report_status.json',{'status':'failed','version':'online_full_v1','error_type':type(e).__name__,'time':time.time()});raise
