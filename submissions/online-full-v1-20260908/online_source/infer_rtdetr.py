...
def main():
    torch.set_num_threads(2);status=ROOT/'candidate_online_20260908/rtdetr_inference_status.json'
    ...
    for name in ['adamw','sgd_momentum']:
        source=ROOT/'checkpoints'/('rtdetr_'+name);out=ROOT/'candidate_online_20260908/rtdetr_holdout'/name;out.mkdir(parents=True,exist_ok=True)
...
if __name__=='__main__':
    try:main()
    except Exception as e:dump(ROOT/'candidate_online_20260908/rtdetr_inference_status.json',{'status':'failed','error_type':type(e).__name__,'time':time.time()});raise
