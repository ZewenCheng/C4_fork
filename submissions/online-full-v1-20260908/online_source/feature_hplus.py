...
def main():
    torch.set_num_threads(2);status=ROOT/'candidate_online_20260908/hplus_features_status.json'
    ...
    for optimizer in ['adamw','sgd_momentum']:
        ...
        output=ROOT/'candidate_online_20260908/hplus_distilled_features'/optimizer;output.mkdir(parents=True,exist_ok=True)
...
if __name__=='__main__':
    try:main()
    except Exception as e:dump(ROOT/'candidate_online_20260908/hplus_features_status.json',{'status':'failed','error_type':type(e).__name__,'time':time.time()});raise
