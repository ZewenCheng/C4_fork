...
def main():
    torch.set_num_threads(2);status=ROOT/'hplus_features_status.json'
    rows=json.loads((ROOT/'data/manifest.json').read_text());vocab=json.loads((ROOT/'data/vocabulary.json').read_text())
    for optimizer in ['adamw','sgd_momentum']:
        source=ROOT/'checkpoints'/('dinov3_hplus_distilled_'+optimizer)
        output=ROOT/'hplus_distilled_features'/optimizer;output.mkdir(parents=True,exist_ok=True)
        ...
        for row in rows:
            ...
            with np.load(output/(row['sample_id']+'.npz')) as d:correct+=int(int(d['scores'].argmax()) in row['label_ids']);total+=1
        dump(output/'complete.json',{'status':'all_features_and_whole_holdout_inference_complete','images':len(rows),'holdout_images':total,'top1_in_annotation_labelset':correct/max(total,1),'checkpoint_sha256':digest,'fusion_training':'pending','time':time.time()})
...
if __name__=='__main__':
    try:main()
    except Exception as e:dump(ROOT/'hplus_features_status.json',{'status':'failed','error_type':type(e).__name__,'time':time.time()});raise
