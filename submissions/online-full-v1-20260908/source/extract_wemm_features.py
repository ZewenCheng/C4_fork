...
def main():
    torch.set_num_threads(2);status=ROOT/'wemm_features_status.json';directory=ROOT/'models/WeMM-Embedding-9B'
    rows=json.loads((ROOT/'data/manifest.json').read_text());processor=AutoProcessor.from_pretrained(directory,local_files_only=True)
    ...
    for optimizer in ['adamw','sgd_momentum']:
        source=ROOT/'checkpoints'/('wemm9b_'+optimizer);out=ROOT/'wemm_features'/optimizer;out.mkdir(parents=True,exist_ok=True)
        ...
        dump(out/'source.json',{'checkpoint_sha256':digest,'base_manifest_sha256':sha(directory/'source_manifest.json'),'data_manifest_sha256':sha(ROOT/'data/manifest.json'),'views':['full_max512','brightness_1.05_max512'],'pooling':'官方embedding方法，末位embedding token','supervision_input':False,'holdout_use':'仅图像推理，不参与融合拟合'})
...
if __name__=='__main__':
    try:main()
    except Exception as e:dump(ROOT/'wemm_features_status.json',{'status':'failed','error_type':type(e).__name__,'time':time.time()});raise
