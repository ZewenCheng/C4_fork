...
from mask_gateway import MaskGateway,MASK_TOOLS,ROOT
from download_assets import sha

GROUNDING_TOOLS=MASK_TOOLS+[{'type':'function','function':{'name':'inspect_grounding','description':'读取训练后Grounding DINO的图像区域候选，框为归一化坐标；不代表病害掩码或实测尺寸。','parameters':{'type':'object','properties':{'sample_id':{'type':'string'}},'required':['sample_id'],'additionalProperties':False}}}]

class GroundingGateway(MaskGateway):
    def __init__(self):
        super().__init__();self.grounding_evidence={}
    def dispatch(self,name,args):
        if name!='inspect_grounding':return super().dispatch(name,args)
        ...
        for optimizer in ['adamw','sgd_momentum']:
            folder=ROOT/'gateway_outputs/grounding_holdout'/optimizer
            ...
            source=ROOT/'checkpoints'/('grounding_dino_'+optimizer)
        ...
        self.grounding_evidence[eid]=result
