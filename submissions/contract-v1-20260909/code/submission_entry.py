...
def publish(work, output, elapsed):
    ...
    write(output / 'infer_time.json.pending', {'infer_time': milliseconds})
    ...
    published = {'status': 'complete', 'images': len(values), 'result_sha256': digest(targets[0]),
                 'infer_time_sha256': digest(targets[1]), 'infer_time': milliseconds,
                 'output': str(output), 'scope': '本次独立完整入口墙钟时间；恢复含中断等待，复用缓存不作冷启动对照'}
...
def main():
    ...
    with run_lock(WORK_BOUNDARY / 'c4-submission-run.lock'), run_lock(work / 'entry.lock'):
        ...
        state = {'status': 'running', 'first_started_at': old.get('first_started_at', time.time()),
                 'attempt': old.get('attempt', 0) + 1, 'pid': os.getpid()}
        ...
        try:
            ...
            elapsed = time.time() - state['first_started_at']
            published = publish(work, args.output, elapsed)
            state.update(status='complete', result_sha256=published['result_sha256'], elapsed_wall_seconds=elapsed)
